...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    class Revista_Separator extends WP_Customize_Control {

        public $type = 'revista-separator';
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    // Radio Image Custom Control Class.
    class Revista_Custom_Radio_Image_Control extends WP_Customize_Control {

        public $type = 'revista-radio-image';
        ...
        public function render_content() {
            ...
            <div id="input_<?php echo esc_attr($this->id); ?>" class="image revista-radio-image-buttenset">
 ...
 * @access public
 */
class Revista_Customize_Section_Upsell extends WP_Customize_Section {
...
*/
class Revista_Repeater_Controler extends WP_Customize_Control {
    /**
     * The control type.
     *
     * @access public
    ...
    */
    ...
    public $revista_box_label = '';

    public $revista_box_add_control = '';

    public $revista_box_add_button = '';
    ...
    public function __construct( $manager, $id, $args = array(), $fields = array() ) {
        $this->fields = $fields;
        $this->revista_box_label = $args['revista_box_label'] ;
        $this->revista_box_add_control = $args['revista_box_add_control'];
        $this->revista_box_add_button = $args['revista_box_add_button'];
    ...
    public function render_content() {
        ...
        <ul class="revista-repeater-field-control-wrap">
            <?php
            $this->revista_get_fields();
            ?>
        </ul>
        <input type="hidden" <?php esc_attr( $this->link() ); ?> class="revista-repeater-collector" value="<?php echo esc_attr( $this->value() ); ?>" />
        <?php

        if( isset( $this->revista_box_add_button ) && $this->revista_box_add_button ){ ?>
            
            <button type="button" class="button revista-add-control-field"><?php echo esc_html( $this->revista_box_add_control ); ?></button>
        <?php
        }
    ...
    private function revista_get_fields(){
        ...
        if( is_array( $values ) ){
            ...
            foreach($values as $value){ ?>
                ...
                <li class="revista-repeater-field-control">

                <div class="title-rep-wrap">
                    <span class="dashicons dashicons-move twp-filter-icon"></span>
                    <h3 class="revista-repeater-field-title"><?php echo esc_html( $this->revista_box_label ); ?></h3>
                    <span class="dashicons dashicons-arrow-down twp-filter-icon"></span>
                </div>

                <div class="revista-repeater-fields">
                    ...
                    foreach ($fields as $key => $field) {
                        ...
                        <div class="revista-fields revista-type-<?php echo esc_attr($field['type']).' '.esc_attr( $class ); ?>">
                            ...
                            switch ($field['type']) {
                                ...
                                case 'selector_color':
                                    ...
                                    $options = $field['options'];
                                    echo '<div class="revista-type-radio">';
                                ...
                                case 'upload':
                                        ...
                                        esc_html_e('No image selected', 'revista');
                                        echo '</div>';
                                        echo '<div class="thumbnail thumbnail-image">';
                                        echo $image;
                                        echo '</div>';
                                        echo '<div class="actions clearfix">';
                                        echo '<button type="button" class="button twp-img-delete-button align-left">'.esc_html__('Remove', 'revista').'</button>';
                                        echo '<button type="button" class="button twp-img-upload-button alignright">'.esc_html__('Select Image', 'revista').'</button>';
                    ...
                    <div class="clearfix revista-repeater-footer">
                        <div class="alignright">
                        <?php if( isset( $this->revista_box_add_button ) && $this->revista_box_add_button ){ ?>
                            <a class="revista-repeater-field-remove" href="#remove"><?php esc_html_e('Delete', 'revista') ?>|</a> 
                        <?php } ?>
                        <a class="revista-repeater-field-close" href="#close"><?php esc_html_e('Close', 'revista') ?></a>
                        </div>
                    </div>
                </div>
                </li>
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    // Repeator Info
    class Revista_Info_Notiece_Control extends WP_Customize_Control {
            ...
            <span class="customize-control-title">
                ...
                <div class="twp-info-icon">
                    <div class="icon-notice-wrap">
                        <span class="dashicons dashicons-move twp-filter-icon"></span>
                        <p><?php esc_html_e( 'Click hold and drag to re-order.', 'revista' ); ?></p>
                    </div>
                    <div class="icon-notice-wrap">
                        <span class="dashicons dashicons-arrow-down twp-filter-icon"></span>
                        <p><?php esc_html_e( 'Click to expand settings.', 'revista' ); ?></p>
                    </div>
                </div>
            </span>
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    // Repeator Info
    class Revista_Premium_Notiece_Control extends WP_Customize_Control {
            ...
            <span class="customize-control-title">
                <div class="theme-info-icon">
                    <div class="icon-notice-wrap">
                        ...
                        <span><?php echo esc_html__('More ','revista'). esc_html( $this->label ). esc_html__(' Available on Premium Version.', 'revista' ); ?></span>
                    </div>
                </div>
            </span>
