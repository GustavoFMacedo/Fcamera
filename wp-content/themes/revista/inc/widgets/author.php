 ...
 * @package Revista
 */
if (!function_exists('revista_author_widgets')) :
    /**
     * Load widgets.
     *
     * @since 1.0.0
     */
    function revista_author_widgets()
    {
        // Auther widget.
        register_widget('Revista_Author_widget');
    }
endif;
add_action('widgets_init', 'revista_author_widgets');

if (!class_exists('Revista_Author_widget')) :
    /**
     * Author widget Class.
     *
     * @since 1.0.0
     */
    class Revista_Author_widget extends Revista_Widget_Base
    {
        /**
        ...
        {
            $opts = array(
                'classname' => 'revista_author_widget',
                'description' => esc_html__('Displays authors details in post.', 'revista'),
            ...
            $fields = array(
                ...
                'image_bg_url' => array(
                    'label' => esc_html__('Widget Background Image:', 'revista'),
                    'type' => 'image',
                ),
                'author-name' => array(
                    'label' => esc_html__('Name:', 'revista'),
                    'type' => 'text',
                    'class' => 'widefat',
                ),
                'description' => array(
                ...
                'image_url' => array(
                    'label' => esc_html__('Author Image:', 'revista'),
                    'type' => 'image',
                ),
                'url-fb' => array(
            ...
            parent::__construct('revista-author-layout', esc_html__('Revista: Sidebar Author Widget', 'revista'), $opts, array(), $fields);
        }
                ...
                <div class="author-social-profiles">
                    <?php if (!empty($params['url-fb'])) { ?>
                        <a href="<?php echo esc_url($params['url-fb']); ?>" target="_blank" class="author-social-icon author-social-facebook">
                            <span class="btn__content" tabindex="-1">
                                <?php revista_theme_svg('facebook'); ?>
                            </span>
                        </a>
                    <?php } ?>
                    ...
                    <?php if (!empty($params['url-lt'])) { ?>
                        <a href="<?php echo esc_url($params['url-lt']); ?>" target="_blank" class="author-social-icon author-social-linkedin">
                             <span class="btn__content" tabindex="-1">
                                <?php revista_theme_svg('linkedin'); ?>
                             </span>
                        </a>
                    <?php } ?>
                    <?php if (!empty($params['url-ig'])) { ?>
                        <a href="<?php echo esc_url($params['url-ig']); ?>" target="_blank" class="author-social-icon author-social-instagram">
                             <span class="btn__content" tabindex="-1">
                                <?php revista_theme_svg('instagram'); ?>
                             </span>
                        </a>
                    <?php } ?>
                </div>
